...
def get_status(httpClass, url_dsf_se, id):
    query = url_dsf_se + "/se/commands/status/" + id
    response = httpClass.get(query)
...
def post_simulation(httpClass, url_dsf_se):
    query = url_dsf_se + "/se/simulation/"
    response = httpClass.put(query + "simulation", topology, "json")
    json_response = response.json()
...
def start_simulation(httpClass, url_dsf_se, id):
    "/se/commands/run/{{id}}"
    query = url_dsf_se + "/se/commands/run/" + id
    payload = {
	"sim_duration_in_hours":48
	}
    response = httpClass.put(query, payload)
    json_response = response.json()
    # logger.debug("type json_response "+str(type(json_response)))
    if response.status_code == 200:
        return json_response
...
def main():
    url_dsf_se = "http://localhost:9091"
    httpClass = Http_commands()
    """basis_name = os.path.join(os.getcwd(),"inputs","Bolzano")
    for name in files_names:
        filepath = os.path.join(basis_name, name)
        logger.debug("filepath "+str(filepath))"""
    for id in id_list:
        logger.debug("id "+str(id))
        answer = start_simulation(httpClass, url_dsf_se, id)
        logger.debug("Answer from start "+str(answer))
        if not answer==1:
            time.sleep(900)
            percentage = 0
            first = True
            while not percentage == 100:
                if not first:
                    ...
                    first = False
                percentage = get_status(httpClass, url_dsf_se, id)
                logger.debug("percentage "+str(percentage)+" for id "+str(id))
                
                
        else:
            logger.error("Error while starting "+str(id))
        #sys.exit(0)
...
def main_plot():
    ...
    dict_with_id_single_ev = {}
    dict_with_id_single_ev["self-consumption"] = {}
    dict_with_id_single_ev["self-production"] = {}
    dict_with_id_single_ev["min-costs"] = {}
    logger.debug("len file names "+str(len(files_names)))
    for i in range(12):
        dict_with_id_single_ev["self-consumption"][files_names[i]] = id_list_self_consumption_vc[i]
        dict_with_id_single_ev["self-production"][files_names[i]] = id_list_self_production_vc[i]
        dict_with_id_single_ev["min-costs"][files_names[i]] =  id_list_min_costs_vc[i]
