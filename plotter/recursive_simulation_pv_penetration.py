...
def read_file_csv():
    import csv
    data_file = "./id_info_journal_2.csv"
    data_list=[]
    with open(data_file, mode='r') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                line_count += 1
            data_list.append({"id":row["ID"], "name":row["File_name"], "control":row["ESS_control"],})
            #print(
...
def main():
    ...
    for element in data_list:
        ...
        logger.debug("#################################################################################")
        id = element["id"]
        complex_name = element["name"] + "_" + element["control"]
        if "Min_Bill" in complex_name or "Max_Self_Con" in complex_name:
            folder_name = os.path.join(os.getcwd(), "results", "pv_penetration_journal", complex_name)
